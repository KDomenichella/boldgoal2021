...
// https://developer.mozilla.org/en-US/docs/Learn/HTML/Forms/Sending_f8orms_through_JavaScript
function sendData() {
  ...
  // radio buttons
  let radio = document.getElementsByName("calltype");
  for (var i = 0, length = radio.length; i < length; i++) {
    if (radio[i].checked) {
      urlEncodedDataPairs.push(
        encodeURIComponent("calltype") + "=" + encodeURIComponent(radio[i].value)
      );
    }
  }

  // dropdown menu
  var dropdown = form.querySelector("[name='mtgtime']");
  urlEncodedDataPairs.push(
    encodeURIComponent("mtgtime") +
      "=" +
  ...
  XHR.addEventListener("load", function(event) {
    if (XHR.readyState === XHR.DONE) {
      if (XHR.status === 200) {
        alert("Thanks for your meeting request!");
      } else {
